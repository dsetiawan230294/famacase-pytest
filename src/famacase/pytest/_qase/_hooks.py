...
def pytest_configure(config):
    ...
    # NOTE (famacase vendor): qase-python-commons 2.x took
    # ``QaseCoreReporter(config_manager, 'pytest', 'qase-pytest')`` but on
    # 3.x the signature was reduced to ``QaseCoreReporter(config_manager)``.
    # Try the newer signature first, fall back to the older one for 2.x.
    try:
        reporter = QaseCoreReporter(config_manager)
    except TypeError:
        reporter = QaseCoreReporter(config_manager, "pytest", "famacase-pytest")  # type: ignore[call-arg]
...
def _add_markers(config):
    ...
    config.addinivalue_line(
        "markers",
        "qase_project_id: mark test to be associated with Qase TestOps test cases in a specific project",
    )
    ...
    config.addinivalue_line(
        "markers",
        "qase_muted: mark test as muted so it will not affect test run status",
    )
    ...
    config.addinivalue_line(
        "markers",
        "qase_ignore_parameters: mark test to ignore specific parameters in Qase reports",
    )
...
def setup_config_manager(config):
    ...
    for option in config.option.__dict__:
        ...
        if option.startswith("qase_"):
            ...
            if (
                option == "qase_environment"
                and config.option.__dict__[option] is not None
            ):
                config_manager.config.set_environment(config.option.__dict__[option])

            if (
                option == "qase_profilers"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_exclude_params"
                and config.option.__dict__[option] is not None
            ):
                config_manager.config.set_exclude_params(
                    [
                        param.strip()
                        for param in config.option.__dict__[option].split(",")
                    ]
                )

            if (
                option == "qase_execution_plan_path"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_testops_run_description"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_testops_run_complete"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_report_connection_local_format"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_testops_batch_size"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_pytest_capture_logs"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_pytest_xfail_status"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_pytest_xpass_status"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_testops_configurations_values"
                and config.option.__dict__[option] is not None
            ):
                ...
                for pair in config_pairs:
                    if "=" in pair:
                        ...
                        config_manager.config.testops.configurations.add_value(
                            name.strip(), config_value.strip()
                        )

            if (
                option == "qase_testops_configurations_create_if_not_exists"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_testops_status_filter"
                and config.option.__dict__[option] is not None
            ):
                config_manager.config.testops.set_status_filter(
                    [
                        status.strip()
                        for status in config.option.__dict__[option].split(",")
                    ]
                )

            if (
                option == "qase_testops_show_public_report_link"
                and config.option.__dict__[option] is not None
            ):
            ...
            if (
                option == "qase_testops_run_external_link_type"
                and config.option.__dict__[option] is not None
            ):
                if not config_manager.config.testops.run.external_link:
                    from qase.commons.models.external_link import ExternalLinkConfig

                    config_manager.config.testops.run.external_link = (
                        ExternalLinkConfig()
                    )
                config_manager.config.testops.run.external_link.set_type(
                    config.option.__dict__[option]
                )

            if (
                option == "qase_testops_run_external_link_url"
                and config.option.__dict__[option] is not None
            ):
                if not config_manager.config.testops.run.external_link:
                    from qase.commons.models.external_link import ExternalLinkConfig
                ...
                config_manager.config.testops.run.external_link.set_link(
                    config.option.__dict__[option]
                )

            if (
                option == "qase_logging_console"
                and config.option.__dict__[option] is not None
            ):
                config_manager.config.logging.set_console(
                    config.option.__dict__[option]
                )

            if (
                option == "qase_logging_file"
                and config.option.__dict__[option] is not None
            ):
    ...
    # NOTE (famacase vendor): ``LoggingOptions`` exists only on qase-python-commons 2.x;
    # on 3.x the logger no longer accepts a ``logging_options`` kwarg. Skip silently
    # when the API isn't available so we stay compatible with both major versions.
    try:
        from qase.commons.logger import LoggingOptions  # type: ignore[attr-defined]
    except ImportError:
        LoggingOptions = None  # type: ignore[assignment]
    if LoggingOptions is not None:
        logging_options = LoggingOptions(
            console=(
                config_manager.config.logging.console
                if config_manager.config.logging.console is not None
                else True
            ),
            file=(
                config_manager.config.logging.file
                if config_manager.config.logging.file is not None
                else config_manager.config.debug
            ),
        )
        try:
            config_manager.logger = config_manager.logger.__class__(
                debug=config_manager.config.debug, logging_options=logging_options
            )
        except TypeError:
            pass
...
def pytest_unconfigure(config):
    # NOTE: Upstream qase-pytest had `getattr(config, "src", None)` here,
    # which was a typo — the attribute set in `pytest_configure` is
    # `config.qase`. Fixed in the famacase-pytest vendored copy.
