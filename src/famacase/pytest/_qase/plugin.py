...
class QasePytestPlugin:
    ...
    def pytest_runtest_makereport(self, item, call):
        ...
        if self.reporter.config.framework.pytest.capture_logs and (
            call.when == "call" or (call.when == "setup" and report.failed)
        ):
        ...
        if (
            call.when == "call"
            and self.__is_xfail_mark(report)
            and self.runtime.result.execution.status != PYTEST_TO_QASE_STATUS["PASSED"]
        ):
    ...
    def _handle_failed_test(self, call):
        ...
        status = (
            ...
            else PYTEST_TO_QASE_STATUS["BROKEN"]
        )
    ...
    def _handle_skip_message_in_setup(self, report, call):
        ...
        if (
            call.when == "setup"
            and report.longrepr
            and self.runtime.result.execution.status == PYTEST_TO_QASE_STATUS["SKIPPED"]
        ):
    ...
    def _set_tags(self, item) -> None:
        ...
        add_tags = getattr(result, "add_tags", None)
        if callable(add_tags):
            add_tags(tags)
            return
        existing = list(getattr(result, "tags", []) or [])
        existing.extend(tags)
        try:
            setattr(result, "tags", existing)
        except (AttributeError, TypeError):
            pass
            ...
            if project_id_markers:
                ...
                for marker in project_id_markers:
                    ...
                    if project_code and testops_ids:
                        self.runtime.result.set_testops_project_mapping(
                            project_code, testops_ids
                        )
            else:
    ...
    def __build_folder_name(item):
        ...
        path_parts.append(QasePytestPlugin.__sanitize_path_component(item.originalname))
    ...
    def __sanitize_path_component(component):
        return (
            component.replace(os.sep, "-").replace(".", "-").replace("_", "-").lower()
        )
